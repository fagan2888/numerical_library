% Third order convergent Riemann-Liouville discrete 
...
for i = 1:N+1
    dp_actual(i) = r_sinusoid(t(i),pf(i),A,w,'int','cos');
end
...
for n = 1:N
    ...
    % compute operator
    dp_approx(n+1) = ((h^(p))/gamma(3+p))*kernel_sum + ((t(n+1)^p)/gamma(p+1))*f(1);
    
end

% compute and display global error
global_error = sum(abs(dp_actual-dp_approx))/(N+1);
